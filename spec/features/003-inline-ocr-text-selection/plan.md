...
3. **`src/components/canvas/TextRegionsOverlay.vue`** (new): props `imgWidth` / `imgHeight`; emits `close`. Structure, from bottom to top:
   ...
   - a single discreet close control pinned to a corner of the wrapper, `aria-label`ed, the only interactive chrome the mode adds. No action bar, no copy button: copying is the browser's native selection copy.
4. **`src/components/canvas/CanvasArea.vue`**: mount `<TextRegionsOverlay v-if="editor.ocrSelectionActive && displayW > 0" … @close="editor.endOcrSelection()" />` alongside the existing overlays; short-circuit `onWheel` and `onContainerClick` while the session is active so zoom/pan don't fight text selection.
...
- **No "Copy all text" button**: it is exactly the added chrome this design avoids; native selection copy covers the case, and the button can return as its own feature if it's missed.
